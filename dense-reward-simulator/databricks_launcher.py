...
# MAGIC %md
# MAGIC # PivotRL Souffle Challenge — Headless Launcher (SIMULATOR-VERIFIER)
# MAGIC
# MAGIC Runs the full **comparative turn-level GRPO** demo end-to-end and prints the
# MAGIC side-by-side table (Base / SFT / End-to-End GRPO / PivotRL).
# MAGIC
# MAGIC **What's different in this folder:** the per-turn reward is produced by an
# MAGIC executable soufflé **simulator** (`souffle_simulator.py`) via *simulate-and-
# MAGIC compare* against the SFT expert action — a real functional verifier — instead
# MAGIC of the keyword lookup used in `../dense-reward/`. Same pivot structure and
# MAGIC results; the reward is just derived faithfully (SFT expert → verifier → 0/1).
# MAGIC
# MAGIC > Requires all three modules co-located here: `souffle_simulator.py`,
# MAGIC > `kitchen_env.py`, `train_comparative_grpo.py`.
# MAGIC
